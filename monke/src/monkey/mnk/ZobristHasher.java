...
public class ZobristHasher implements Cloneable {
    ...
    public enum Symmetry {
        ...
        C {
            public Position apply(Position p) {
                return p == null ? null : new Position(p.ROWSNUMBER, p.COLUMNSNUMBER, p.ROWSNUMBER - p.getRow() - 1, p.COLUMNSNUMBER - p.getColumn() - 1);
            }
        ...
        F {
            public Position apply(Position p) {
                return p == null ? null : new Position(p.ROWSNUMBER, p.COLUMNSNUMBER, p.getRow(), p.COLUMNSNUMBER - p.getColumn() - 1);
            }
        ...
        G {
            public Position apply(Position p) {
                if (p == null) return null;
                // validateSquareBoard(p);
                return new Position(p.ROWSNUMBER, p.COLUMNSNUMBER, p.ROWSNUMBER - p.getColumn() - 1, p.COLUMNSNUMBER - p.getRow() - 1);
            }
        ...
        H {
            public Position apply(Position p) {
                return p == null ? null : new Position(p.ROWSNUMBER, p.COLUMNSNUMBER, p.ROWSNUMBER - p.getRow() - 1, p.getColumn());
            }
    ...
    public void addOrRemove(Position position, Player player) {
        ...
        for (Symmetry s : symmetries) {
            ...
            if (symmetryUsed == null || candidate < hashCodeCandidates[symmetryUsed.ordinal()]) symmetryUsed = s;
        }
    }
